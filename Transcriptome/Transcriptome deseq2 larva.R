...
aa <- import.gff3("0.Drosophila_melanogaster.BDGP6.28.100.chr.gff3.gz")
head(aa)
...
for (i in 1:nrow(tmp_table)){
  if(i == 1){ tmp_table[i,c(2,3)] =  c("larva-AP-F", "larva-AP-M") }
  if(i == 2){ tmp_table[i,c(2,3)] =  c("larva-APX-F", "larva-DF-F") }
  if(i == 3){ tmp_table[i,c(2,3)] =  c("larva-GF-F", "larva-GF-M") }
  if(i == 4){ tmp_table[i,c(2,3)] =  c("larva-AP-F", "larva-GF-F") }
}
...
deseq_run_with_letter = function(r.num.input, txi.salmon.input){
    #preprocessing
    res05.list = list()
    res.list = list()
    ...
    if(r.num.input %in% c(1:4)){
      ...
      if(is.null(gse) == FALSE){
        ggsave(plot = cnetplot(gse, categorySize="pvalue", foldChange=gene_list, circular = T, colorEdge = T), filename = paste0(r.num.input,"_",letter[1],"Vs",letter[2],"_gseGO_cnet_all_circular.tiff"),
...
} # must delete un-involved groups # must delete column 'order' inside the csv file
