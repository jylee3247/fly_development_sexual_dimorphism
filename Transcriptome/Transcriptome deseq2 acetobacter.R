...
install.packages("rlang")# if update error
install.packages("xfun")# if update error
install.packages("dplyr")# if update error
...
# txdb processing
txdb <- makeTxDbFromGFF(file="Acetobacter_pasteurianus_subsp_pasteurianus_lmg_1262_nbrc_106471.ASM24162v2.46.gff3.gz")
transcripts(txdb)
keytypes(txdb)
columns(txdb)
k = keys(txdb, keytype = "CDSNAME")
head(k)
tx2gene <- AnnotationDbi::select(txdb, k, "GENEID", "CDSNAME") 
